...
def run_trial(parameters, train_set=None, test_sets=None):
    experiment_name, experiment_features_list, exp_settings = parameters
    experiment_name = experiment_name + '_' + train_set #append f or g to experiment name, depending on training set used
    ...
    for test_indx in test_indices:
        ...
        os.path.exists(os.path.join(test_path, name_append_test, 'results.p'))):
            ...
            command = [py_env, './run_json_f_or_g.py'] + arg_list
            print(command)
            print('\n *** \n')
...
#TRAIN ON F - TEST ON ALL 3 SETS
for params in param_list:
    run_trial(params, train_set='train_on_f',test_sets=['test', 'test_on_f', 'test_on_g'])

#TRAIN ON G - TEST ON ALL 3 SETS
for params in param_list:
    run_trial(params, train_set='train_on_g', test_sets=['test', 'test_on_f', 'test_on_g'])
