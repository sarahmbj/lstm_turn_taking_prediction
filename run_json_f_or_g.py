...
#    import h5py
from data_loader_f_or_g import TurnPredictionDataset
...
# training data loader - train_set selects only f, only g, or both
...
train_dataset = TurnPredictionDataset(feature_dict_list, annotations_dir, train_list_path, sequence_length,
                                      prediction_length, train_set, data_select=data_set_select)
...
if slow_test:
    ...
    for test_set in test_sets:
        # test data loader - for f only, g only, or both - create for all the test sets that are required
        if test_set == 'test_on_f':
            test_on_f_dataset = TurnPredictionDataset(feature_dict_list, annotations_dir, test_list_path, sequence_length,
                                                 prediction_length, 'test_on_f', data_select=data_set_select)
            test_on_f_dataloader = DataLoader(test_on_f_dataset, batch_size=1, shuffle=False, num_workers=0, drop_last=False,
                                         pin_memory=p_memory)
        elif test_set == 'test_on_g':
            test_on_g_dataset = TurnPredictionDataset(feature_dict_list, annotations_dir, test_list_path, sequence_length,
                                                 prediction_length, 'test_on_g', data_select=data_set_select)
    ...
    # quick test loader - not modified to allow f and g - will just default to using both
    test_dataset = TurnPredictionDataset(feature_dict_list, annotations_dir, test_list_path, sequence_length,
...
for epoch in range(0, num_epochs):
    ...
    t_epoch_end = t.time()
    model.eval()
    test() #TODO: check this won't break the training!!!!
